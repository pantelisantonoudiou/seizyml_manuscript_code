...
def should_skip(state_df, fold, fraction, rand_fraction, feature_type, feature_set, model_name):
    if len(state_df) == 0:
        return False
    return not state_df[(state_df['fold'] == fold) &
                        (np.isclose(state_df['training_fraction'], fraction, atol=1e-6)) &
                        (np.isclose(state_df['permuted_fraction'], rand_fraction, atol=1e-6)) &
...
def run_models(norm_type):
    ...
    # ---------------------------- Main settings ---------------------------- #
    # paths
    model_path = os.path.join('data', 'saved_models', norm_type, 'small_models_permute_labels')
    ...
    nleast_corr = 5
    feature_types = ['local']
    data_fractions = np.array([.5, 1, 2.5, 5])/100
    randomize_fraction = np.array([0, 5, 10, 20, 40, 60, 80, 100])/100
    n_models = len(feature_size)*2*nsplits*len(models)*len(feature_types)*len(data_fractions)*len(randomize_fraction)
    # ----------------------------------------------------------------------- #
    ...
    for fraction in data_fractions:
        ...
        for i, (train_index, val_index) in enumerate(cv.split(x_fraction, y_fraction)):# fold
            for feature_type in feature_types: # feature type
                for rand_fraction in randomize_fraction:
                    
                    # randomize a percentage of labels
                    y_randomized = y_fraction.copy()
                    fraction_samples = int(len(y_randomized)*rand_fraction)
                    y_randomized[:fraction_samples] = np.random.permutation(y_randomized[:fraction_samples])
                    ...
                    y_train = y_randomized[train_index]
                    x_val = x_fraction[val_index, :]
                    y_val = y_randomized[val_index]
                    ...
                    for feature_set, sel_features in feature_space.items():
                        for model_name in models:
                            ...
                            if should_skip(state_df, i, fraction, rand_fraction, feature_type, feature_set, model_name):
                            ...
                            model_dict = {'id': model_id,  'fold':i, 'feature_type':feature_type,
                                          ...
                                          'permuted_fraction':rand_fraction,
                                          'hyperparameters':str(search['best_params']),
                                          'fit_metric':search['best_score']
                                          }
