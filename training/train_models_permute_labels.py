...
def should_skip(state_df, fold, fraction, feature_type, feature_set, model_name):
    ...
    return not state_df[(state_df['fold'] == fold) &
                        (np.isclose(state_df['permute_fraction'], fraction, atol=1e-6)) &
...
def run_models(norm_type):
    ...
    nleast_corr = 5
    feature_types = ['local']
    randomize_fraction = [0, 1, 5, 10, 20, 50, 100]
    n_models = len(feature_size)*2*nsplits*len(models)*len(feature_types)*len(randomize_fraction)
    # ----------------------------------------------------------------------- #
    ...
    cntr = 0
    for i, (train_index, val_index) in enumerate(cv.split(x_train_all, y_train_all)):# fold
        for feature_type in feature_types: # feature type
            for fraction in randomize_fraction:
                # randomize a percentage of labels
                y_randomized = y_train_all.copy()
                fraction_samples = int(len(y_randomized)*(fraction/100))
                ...
                for feature_set, sel_features in feature_space.items():
                    for model_name in models:
                        ...
                        model_dict = {'id': model_id,  'fold':i, 'feature_type':feature_type,
                                      'feature_set':feature_set, 'model_name':model_name, 'permute_fraction':fraction,
